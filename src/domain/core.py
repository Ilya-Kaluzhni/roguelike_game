...
from domain.zombie import Zombie
from domain.mimic import Mimic
from domain.vampire import Vampire
from domain.ogre import Ogre
from domain.snake import Snake
from domain.enemy_movement import MoveEnemy
from random import randint, choice
from domain.fight import Fight
from domain.items import Item
from datalayer.datalayer import GameSaveManager, StatisticsManager
import time
...
class GameController:
    ...
    def spawn_items(self):
        ...
        while num_items:
            ...
            exit_cell = None
            for room in self.level[0]:
                if 'exit' in room and room['exit']:
                    exit_cell = room['exit']
                    break
            
            if self.game_map.can_set_smth(*cell) and cell != exit_cell:
                item = choice([
    ...
    def get_game_over_state(self):
        ...
        """
        # Возвращаем полное состояние игры при окончании, включая player
        player_data = self.character.presentation_data()
        player_data['level'] = self.n_level
