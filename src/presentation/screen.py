# /Users/ilya/roguelike_game/src/presentation/screen.py
import curses
...
def main(stdscr):
    ...
    game_map.add_rooms(rooms_ui)
    game_map.add_corridors(corridors_ui)

    # стартовая позиция игрока — центр первой комнаты
    player_pos = (rooms_ui[0]['x'] + rooms_ui[0]['width']//2, rooms_ui[0]['y'] + rooms_ui[0]['height']//2)
    ...
    if next_step == MenuId.NEW_GAME.value:
        ...
        while True:
            ...
            # --- движение игрока с проверкой границ ---
            x, y = player_pos
            if key in Keys.W_UP.value:
                player_pos = (x, max(0, y - 1))
            elif key in Keys.A_LEFT.value:
                player_pos = (max(0, x - 1), y)
            elif key in Keys.S_DOWN.value:
                player_pos = (x, min(height - 1, y + 1))
            elif key in Keys.D_RIGHT.value:
                player_pos = (min(width - 1, x + 1), y)

            win_game.update(player_pos, monsters, items)
            win_rules.press_btn(key)

            # --- использование предметов ---
            if key in Keys.H_USE_WEAPON.value:
